...
import 'package:Shepower/OnBoarding/Citizen/FaceDetect1.dart';
import 'package:Shepower/OnBoarding/Citizen/otpScreen1.dart';
import 'package:Shepower/OnBoarding/Citizen/otpScreen11.dart';
...
class _LoginState extends State<Login1> {
  ...
  Future<void> secondLogin() async {
    ...
    var request = http.Request(
        'POST', Uri.parse('${ApiConfig.baseUrl}loginViaOtpcitizen'));
  ...
  Future<void> loginUser() async {
    ...
    try {
      ...
      var request = http.Request(
          'POST', Uri.parse('${ApiConfig.baseUrl}registrationcitizen'));
      request.body = json.encode({
        "mobilenumber": mobileNumber,
        "token": fcmToken,
        "device_id": deviceId
      ...
      if (message == 'You have already registerd with the Shepower') {
        ...
        if (data['istherearenot']['adminBlock'] == true) {
          String msg = 'You have been blocked by ShePower Team';
          // ignore: use_build_context_synchronously
          showdialogueblocked(context, msg);
        } else {
          secondLogin();
        ...
        showregisterd(
          context,
          "Already Registered in Leader Please  login",
        );
  ...
  void navigatetoCitizenProfile() {
    Navigator.push(
        context, MaterialPageRoute(builder: (context) => const Facescan1()));
  }
